...
const VERSION = '18.2.3-BETA';
const CACHE_KEY = `logistics_v18_2_3_beta_shared_final_`;
...
export const renderDashboard = async (container, user, onLogout) => {
  ...
  */
      ...
      <div class="topbar-brand">
        <div style="display:flex; align-items:center; gap:10px;">
          <h2 style="font-weight:700; color:#fff; display:flex; align-items:center; gap:8px;">
            ...
            <span style="font-size:12px; color:rgba(255,255,255,0.4); font-weight:400; margin-left:5px;">v18.2.3</span>
          </h2>
        </div>
      </div>
  ...
  const processReporteUCA = (matriz, reserva) => {
    ...
    const results = [];
    matriz.forEach(m => {
      const ubi = String(m.UBICACION || '').trim();
      if (!ubi) return;
