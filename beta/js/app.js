import { getSession, logout } from './services/auth.js?v=11.3.0';
...
class App {
  ...
  async navigate() {
    const user = getSession();
    const versionStr = "v11.3.0-dev [BETA]";
    ...
    try {
        ...
        if (user) {
            const { renderDashboard } = await import(`./views/dashboard_v6.js?v=11.3.0_${timestamp}`);
            this.root.innerHTML = '';
