import { getSession, logout } from './services/auth.js?v=11.2.2';
...
class App {
  ...
  async navigate() {
    const user = getSession();
    const versionStr = "v11.2.2-dev [BETA]";
    ...
    try {
        ...
        if (user) {
            const { renderDashboard } = await import(`./views/dashboard_v6.js?v=11.2.2_${timestamp}`);
            this.root.innerHTML = '';
