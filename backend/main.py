...
def emergency_rescue():
    """
    Función de rescate para cuando la DB llega al límite de 1GB.
    """
    if not os.path.exists(DB_PATH): return
    
    size_mb = os.path.getsize(DB_PATH) / (1024*1024)
    if size_mb > 900: # Si estamos por encima de 900MB
        print(f"🚨 RESCATE: DB muy pesada ({size_mb}MB). Rotando archivo...")
        backup_path = DB_PATH.replace(".db", f"_backup_{datetime.now().strftime('%H%M%S')}.db")
        
        try:
            # 1. Intentamos liberar espacio interno sin duplicar archivo
            conn = sqlite3.connect(DB_PATH)
            cursor = conn.cursor()
            cursor.execute("PRAGMA journal_mode = OFF")
            cursor.execute("DELETE FROM audit_logs") # Limpiar logs pesados
            cursor.execute("DELETE FROM shared_data WHERE updated_at < date('now', '-7 days')")
            conn.commit()
            conn.close()
            print("✅ Limpieza interna completada.")
        except Exception as e:
            print(f"❌ Falló limpieza interna: {e}")
