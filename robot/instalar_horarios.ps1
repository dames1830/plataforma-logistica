...
$bat = @(
  ...
  'REM  OJO: en cmd.exe "if errorlevel N" significa MAYOR O IGUAL que N, asi que',
  'REM  el 2 se comprueba ANTES que el 1 o se lo come la rama de "no le toca".',
  'REM  Codigo 2 = la tarea no existe en el horario. Antes caia en la misma rama',
  'REM  que "no le toca" y devolvia 0: el ASN se paso dos dias sin bajar con',
  'REM  Windows marcando exito todos los dias. Ahora la tarea queda en rojo.',
  'if errorlevel 2 (',
  '  echo [HORARIO] ERROR DE INSTALACION: la tarea %~1 no existe en el horario.',
  '  exit /b 2',
  ')',
  'if errorlevel 1 (',
