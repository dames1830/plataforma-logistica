...
const seccion = (n, titulo, bajada, cuerpo) => `
  <div class="glass-panel" style="padding:0; overflow:hidden; border:1px solid rgba(255,255,255,0.07); margin-bottom:1.1rem;">
    <div style="padding:1rem 1.3rem; border-bottom:1px solid rgba(255,255,255,0.06); display:flex; align-items:center; gap:11px;">
      <span style="background:rgba(99,102,241,0.18); color:#a5b4fc; width:26px; height:26px; border-radius:8px; display:flex; align-items:center; justify-content:center; font-size:0.78rem; font-weight:900; flex:none;">${n}</span>
      <div>
        <h3 style="margin:0; color:#fff; font-size:0.92rem; font-weight:900; letter-spacing:0.4px;">${titulo}</h3>
...
const remate = (texto, color = '#4ade80') => `
  <div style="margin-top:0.9rem; padding:0.7rem 1rem; background:rgba(255,255,255,0.03); border-left:3px solid ${color}; border-radius:8px; font-size:0.8rem; color:#fff; font-weight:700; line-height:1.6;">
    ${texto}
  </div>`;
...
const abrible = (texto, tipo, campo, extra = '') => `
  <a href="#" class="pp-num" data-tipo="${tipo}" data-campo="${campo}"
     style="color:inherit; text-decoration:underline dotted rgba(165,180,252,0.7); text-underline-offset:3px; cursor:pointer; ${extra}">${texto}</a>`;
...
const sec1 = (C) => {
    ...
    const caja = (titulo, texto, mov, pie, color) => `
      <div style="flex:1; min-width:260px; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.07); border-top:3px solid ${color}; border-radius:12px; padding:1rem 1.1rem;">
        <div style="font-size:0.85rem; font-weight:900; color:#fff; margin-bottom:0.5rem;">${titulo}</div>
    ...
    return `<div style="display:flex; gap:1rem; flex-wrap:wrap;">
      ${caja('Suelto',
        ...
        `${F(C.mov.suelto)} movimientos · ${pct(C.mov.suelto, total)}% del total`, '', '#38bdf8')}
      ${caja('Prepack',
        'El operario saca una caja cerrada con una curva de tallas adentro. Una ida, varios pares. <b style="color:rgba(255,255,255,0.8);">El sistema anota 1</b>, aunque adentro vayan 10.',
        `${F(C.mov.prepack)} movimientos · ${pct(C.mov.prepack, total)}% del total`,
        `${F(C.mov.cajas)} cajas`, '#f59e0b')}
    </div>`;
};
...
const sec2 = (C, dias) => {
    ...
    const filas = dias.map(d => {
        ...
        return `<tr style="border-bottom:1px solid rgba(255,255,255,0.03);">
          ...
          <td style="padding:0.45rem 0.9rem; text-align:right; color:#fff; font-weight:700;">${F(p.pares_reales)}</td>
          <td style="padding:0.45rem 0.9rem; text-align:right; color:#4ade80; font-weight:700;">+${F(x)}</td>
          <td style="padding:0.45rem 0.9rem; text-align:right; color:var(--text-muted);">${pct(x, p.pares_wms)}%</td>
        </tr>`;
    }).join('');
    return `
      <div style="text-align:center; margin-bottom:1.1rem;">
        <div style="font-size:2.4rem; font-weight:900; color:#4ade80; line-height:1.1;">${F(dif)}</div>
        <div style="font-size:0.78rem; color:var(--text-muted); line-height:1.7;">
          pares que salieron y el reporte no contaba — un <b style="color:#4ade80;">${pct(dif, C.pares_wms)}% más</b> de lo que se creía.
        </div>
      </div>
        ...
        <table style="width:100%; border-collapse:collapse; font-size:0.76rem; color:#d1d5db;">
          ...
          <tbody>${filas}
            <tr style="border-top:2px solid rgba(255,255,255,0.1); font-weight:900; color:#fff;">
              ...
              <td style="padding:0.55rem 0.9rem; text-align:right; color:#4ade80;">+${F(dif)}</td>
    ...
    const barra = (rot, seg, ancho, color) => `
      <div style="margin-bottom:0.7rem;">
        <div style="display:flex; justify-content:space-between; font-size:0.76rem; margin-bottom:4px;">
          <span style="color:rgba(255,255,255,0.75);">${rot}</span>
          <b style="color:${color};">${seg} s</b>
        </div>
        <div style="background:rgba(255,255,255,0.05); border-radius:5px; height:12px; overflow:hidden;">
          <div style="width:${ancho}%; height:100%; background:${color};"></div>
        </div>
      </div>`;
    return `
      ${barra('1 par suelto', s.mediana, 100 * s.mediana / unoAUno, '#38bdf8')}
      ${barra('1 caja de 10 pares', c10.mediana || 0, 100 * (c10.mediana || 0) / unoAUno, '#f59e0b')}
      ${barra('Esos 10 pares, uno por uno', unoAUno, 100, '#ef4444')}
      ${remate(`Una caja de 10 pares no cuesta 10 veces más. Cuesta <b style="color:#4ade80;">${factor} veces más</b>.`)}
      <div style="margin-top:0.8rem; font-size:0.71rem; color:rgba(255,255,255,0.35); line-height:1.7;">
...
const sec4 = (C) => {
    ...
    const fila = (rot, sub, a, b, cuenta) => `
      <tr style="border-bottom:1px solid rgba(255,255,255,0.03);">
        <td style="padding:0.5rem 0.9rem;">${rot}${sub ? `<div style="font-size:0.66rem; color:rgba(255,255,255,0.3);">${sub}</div>` : ''}</td>
        <td style="padding:0.5rem 0.9rem; text-align:right; color:#38bdf8; font-weight:700;">${a} s</td>
        <td style="padding:0.5rem 0.9rem; text-align:right; color:#f59e0b; font-weight:700;">${b} s</td>
        <td style="padding:0.5rem 0.9rem; text-align:right; color:#fff; font-weight:800;">${cuenta}</td>
      </tr>`;

    return `
      <div style="display:flex; gap:1rem; flex-wrap:wrap; margin-bottom:1rem;">
        <div style="flex:1; min-width:240px; background:rgba(255,255,255,0.03); border-radius:10px; padding:0.9rem 1rem;">
          <div style="font-size:0.78rem; font-weight:800; color:#fff;">Ya estaba parado ahí</div>
          <div style="font-size:1.15rem; font-weight:900; margin:4px 0; color:#fff;">${sm.mediana} s <span style="color:rgba(255,255,255,0.3);">/</span> ${pm.mediana} s</div>
        ...
        <div style="flex:1; min-width:240px; background:rgba(255,255,255,0.03); border-radius:10px; padding:0.9rem 1rem;">
          <div style="font-size:0.78rem; font-weight:800; color:#fff;">Tuvo que caminar</div>
          <div style="font-size:1.15rem; font-weight:900; margin:4px 0; color:#fff;">${sc.mediana} s <span style="color:rgba(255,255,255,0.3);">/</span> ${pc.mediana} s</div>
      ...
      ${remate('El trabajo está en <b style="color:#4ade80;">llegar al sitio</b>, no en levantar la caja.')}

      <div style="margin-top:1.2rem; font-size:0.78rem; font-weight:900; color:#fff; letter-spacing:0.4px;">PERO NO CAMINAN LO MISMO</div>
      ...
      <table style="width:100%; border-collapse:collapse; font-size:0.76rem; color:#d1d5db; margin-bottom:1rem;">
        <thead><tr style="color:var(--text-muted); text-align:left;">
        ...
        <tbody>
          <tr style="border-bottom:1px solid rgba(255,255,255,0.03);">
            ...
            <td style="padding:0.45rem 0.9rem; text-align:right;">${F(sm.n)} <span style="color:rgba(255,255,255,0.35);">(${pct(sm.n, totalS)}%)</span></td>
            <td style="padding:0.45rem 0.9rem; text-align:right;">${F(sc.n)} <span style="color:rgba(255,255,255,0.35);">(${pct(sc.n, totalS)}%)</span></td>
          </tr>
          <tr>
            <td style="padding:0.45rem 0.9rem;">Caja de prepack</td>
            <td style="padding:0.45rem 0.9rem; text-align:right;">${F(pm.n)} <span style="color:rgba(255,255,255,0.35);">(${pct(pm.n, totalP)}%)</span></td>
            <td style="padding:0.45rem 0.9rem; text-align:right;">${F(pc.n)} <span style="color:rgba(255,255,255,0.35);">(${pct(pc.n, totalP)}%)</span></td>
          </tr>
        </tbody>
      </table>
      ...
      <div style="margin-top:0.8rem; font-size:0.72rem; color:rgba(255,255,255,0.4); line-height:1.8;">
        ...
        <br><b style="color:rgba(255,255,255,0.6);">Y ahí aparece una palanca de layout:</b> una caja que obliga a moverse tarda
        ${pc.mediana} s; una que se saca sin moverse, ${pm.mediana} s. Son <b style="color:rgba(255,255,255,0.6);">${pc.mediana - pm.mediana} segundos menos por caja</b>.
        Hoy ${F(pc.n)} cajas —el ${pct(pc.n, totalP)}%— obligan a moverse; si el prepack estuviera concentrado en menos
        ubicaciones, ahí hay hasta <b style="color:rgba(255,255,255,0.6);">${((pc.mediana - pm.mediana) * pc.n / 3600).toFixed(1)} horas</b> de trabajo en estas ${C.jornadas || 1} jornadas.
      </div>`;
};
...
const sec5 = (C) => {
    ...
    const filas = curvas.map(c => {
        ...
        return `<tr style="border-bottom:1px solid rgba(255,255,255,0.03);">
          <td style="padding:0.5rem 0.9rem; color:#fff;">${c} pares</td>
          ...
          <td style="padding:0.5rem 0.9rem; text-align:right; font-weight:900; color:${flojo ? 'rgba(255,255,255,0.35)' : '#4ade80'};">${usa ? usa.toFixed(2) : '—'}</td>
          <td style="padding:0.5rem 0.9rem; font-size:0.68rem; color:rgba(255,255,255,0.35);">${flojo ? 'muestra chica: se usa el general' : ''}</td>
        </tr>`;
    }).join('');
    return `
      <table style="width:100%; border-collapse:collapse; font-size:0.77rem; color:#d1d5db;">
        ...
        <tbody>
          <tr style="border-bottom:1px solid rgba(255,255,255,0.06); background:rgba(56,189,248,0.05);">
            <td style="padding:0.5rem 0.9rem; color:#fff; font-weight:700;">1 par suelto</td>
            ...
            <td style="padding:0.5rem 0.9rem; text-align:right; font-weight:900; color:#38bdf8;">1.00</td>
            <td style="padding:0.5rem 0.9rem; font-size:0.68rem; color:rgba(255,255,255,0.35);">la referencia</td>
          </tr>
          ${filas}
        </tbody>
      </table>
      <div style="margin-top:0.8rem; font-size:0.71rem; color:rgba(255,255,255,0.4); line-height:1.8;">
        Un par suelto es la referencia y vale 1,00. Cada tamaño de caja vale lo que tarda, comparado con él.
        <b style="color:#a5b4fc;">Toque cualquier número subrayado</b> para ver de dónde sale.
...
const sec6 = (C, dias) => {
    ...
    const tabla = (c) => {
        const filas = dias.map(d => {
            ...
            return `<tr style="border-bottom:1px solid rgba(255,255,255,0.03);">
              ...
              <td style="padding:0.4rem 0.9rem; text-align:right; color:${pocas ? 'rgba(255,255,255,0.3)' : '#fff'}; font-weight:${pocas ? '400' : '700'};">
        ...
        return `
          <div style="font-size:0.75rem; font-weight:900; color:#f59e0b; letter-spacing:0.5px; margin:1rem 0 0.4rem;">CAJA DE ${c} PARES</div>
          <div style="overflow:auto;">
          <table style="width:100%; border-collapse:collapse; font-size:0.73rem; color:#d1d5db;">
            <thead><tr style="color:var(--text-muted); text-align:left;">
            ...
            <tbody>${filas}
              <tr style="border-top:2px solid rgba(255,255,255,0.1); font-weight:900; color:#fff;">
                ...
                <td style="padding:0.5rem 0.9rem; text-align:right;">${abrible(F(t.n), String(c), 'n')}</td>
                <td style="padding:0.5rem 0.9rem; text-align:right;">${abrible(t.mediana + ' s', String(c), 'seg')}</td>
                <td style="padding:0.5rem 0.9rem; text-align:right; color:#4ade80;">${t.mediana} ÷ ${s.mediana} = ${(t.mediana / s.mediana).toFixed(2)}</td>
              </tr>
            </tbody>
          </table></div>`;
    };
    return `
      <div style="font-size:0.73rem; color:var(--text-muted); line-height:1.8; margin-bottom:0.3rem;">
        ...
        <b style="color:rgba(255,255,255,0.6);">jornada por jornada</b>, para que se vea que el número de arriba no sale de un solo día.
...
const sec8 = (C) => {
    ...
    return `
      <div style="display:flex; gap:1rem; flex-wrap:wrap; margin-bottom:0.9rem;">
        <div style="flex:1; min-width:200px; text-align:center; background:rgba(255,255,255,0.03); border-radius:10px; padding:0.9rem;">
          <div style="font-size:1.5rem; font-weight:900; color:#4ade80;">${F(viajes)}</div>
          <div style="font-size:0.7rem; color:var(--text-muted); line-height:1.6;">viajes ahorrados en ${C.jornadas || 1} jornadas</div>
        </div>
        <div style="flex:1; min-width:200px; text-align:center; background:rgba(255,255,255,0.03); border-radius:10px; padding:0.9rem;">
          <div style="font-size:1.5rem; font-weight:900; color:#fff;">${pct(paresCaja, C.pares_reales)}%</div>
          <div style="font-size:0.7rem; color:var(--text-muted); line-height:1.6;">de los pares sale en caja…</div>
        </div>
        <div style="flex:1; min-width:200px; text-align:center; background:rgba(255,255,255,0.03); border-radius:10px; padding:0.9rem;">
          <div style="font-size:1.5rem; font-weight:900; color:#fff;">${pct(C.mov.prepack, total)}%</div>
      ...
      <div style="font-size:0.75rem; color:rgba(255,255,255,0.55); line-height:1.9;">
        Esos <b style="color:#fff;">${F(paresCaja)} pares</b> salieron en <b style="color:#fff;">${F(C.mov.cajas)} cajas</b>.
...
const panelDetalle = (C, tipo, campo) => {
    ...
    if (campo === 'n') {
        ...
        return `
          <h4 style="margin:0 0 0.2rem; color:#fff; font-size:0.82rem; font-weight:900;">De dónde salen esas ${F(pasos[pasos.length - 1].n)} mediciones</h4>
          ...
          <table style="width:100%; border-collapse:collapse; font-size:0.72rem; color:#d1d5db;">
            <thead><tr style="color:var(--text-muted); text-align:left;">
            ...
            <tbody>
              ${pasos.map((e, i) => `
                <tr style="${i === pasos.length - 1 ? 'border-top:1px solid rgba(255,255,255,0.1); font-weight:800; color:#fff;' : 'border-bottom:1px solid rgba(255,255,255,0.03);'}">
                  <td style="padding:0.35rem 0.7rem;">${esc(e.q)}</td>
                  <td style="padding:0.35rem 0.7rem; text-align:right;"><b>${F(e.n)}</b></td>
                  <td style="padding:0.35rem 0.7rem; font-size:0.67rem; color:rgba(255,255,255,0.35);">${esc(e.p)}</td>
                </tr>`).join('')}
            </tbody>
          </table>`;
    }
    ...
    return `
      <h4 style="margin:0 0 0.2rem; color:#fff; font-size:0.82rem; font-weight:900;">De dónde salen esos ${esc2.mediana} segundos</h4>
      ...
      ${muestras.length ? `
        <table style="width:100%; border-collapse:collapse; font-size:0.72rem; color:#d1d5db; margin-bottom:0.8rem;">
          <thead><tr style="color:var(--text-muted); text-align:left;">
          ...
          <tbody>${muestras.map(m => `
            <tr style="border-bottom:1px solid rgba(255,255,255,0.03);">
              ...
              <td style="padding:0.35rem 0.7rem; font-size:0.67rem; color:rgba(255,255,255,0.35);">${esc(m.ubi)}</td>
            </tr>`).join('')}
          </tbody>
        </table>` : ''}
      ...
      <table style="width:100%; border-collapse:collapse; font-size:0.72rem; color:#d1d5db;">
        <tbody>${esc2.puestos.map(p => `
          <tr style="${p.medio ? 'background:rgba(245,158,11,0.1);' : 'border-bottom:1px solid rgba(255,255,255,0.03);'}">
            <td style="padding:0.35rem 0.7rem; text-align:right; color:var(--text-muted);">la nº ${F(p.pos)}</td>
            <td style="padding:0.35rem 0.7rem; text-align:right;"><b style="${p.medio ? 'color:#f59e0b; font-size:1rem;' : ''}">${p.seg} s</b></td>
            <td style="padding:0.35rem 0.7rem; font-size:0.67rem; color:rgba(255,255,255,0.4);">${p.et}${p.medio ? ' ← el número del cuadro' : ''}</td>
          </tr>`).join('')}
        </tbody>
      </table>
      ${esc2.centro.length ? `
        ...
        <div style="display:flex; gap:6px; flex-wrap:wrap;">
          ${esc2.centro.map((v, i) => `<span style="padding:3px 9px; border-radius:6px; font-size:0.7rem; ${i === Math.floor(esc2.centro.length / 2) ? 'background:rgba(245,158,11,0.2); color:#f59e0b; font-weight:900;' : 'background:rgba(255,255,255,0.05); color:rgba(255,255,255,0.5);'}">${v} s</span>`).join('')}
        </div>` : ''}`;
};
...
export const pintarPrepack = (contenedor, dias) => {
        ...
        contenedor.innerHTML = `
          <div class="glass-panel" style="padding:3rem 2rem; text-align:center; color:var(--text-muted);">
            ...
            <p style="margin:0 auto; max-width:52ch; font-size:0.82rem; line-height:1.7;">
              Este análisis se arma con los archivos de picking. Cargalos en <b style="color:#a5b4fc;">Picking → Archivo Picking</b>.
            </p>
          </div>`;
        return;
    }
    ...
    contenedor.innerHTML = `
      <div style="margin-bottom:1.1rem;">
        <div style="font-size:0.68rem; color:#a5b4fc; font-weight:900; letter-spacing:1.5px;">ANÁLISIS DE PICKING</div>
        <h2 style="margin:2px 0 4px; color:#fff; font-size:1.35rem; font-weight:900;">Prepack contra suelto</h2>
        <div style="font-size:0.76rem; color:var(--text-muted); line-height:1.7;">
          ...
          <b style="color:rgba(255,255,255,0.6);">Nada estimado: todo sale del archivo del WMS.</b>
        </div>
      </div>
    ...
    contenedor.onclick = (ev) => {
        ...
        const caja = `<div style="background:rgba(15,23,42,0.75); border:1px solid rgba(99,102,241,0.3); border-radius:10px; padding:0.9rem 1.1rem; margin:0.5rem 0;">
