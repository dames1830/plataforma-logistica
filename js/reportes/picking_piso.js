...
const quePasa = (f) => {
    ...
    if (!f.picado) return { texto: 'entra y no sale', color: 'var(--warning-soft)' };
    if (!f.almacenado) return { texto: 'sale y no entra', color: 'var(--sky)' };
    const d = Math.abs(f.dif), p = Math.round(100 * d / may);
    if (!p) return { texto: 'parejo', color: 'var(--text-muted)' };
    return f.dif > 0
        ? { texto: `entraron <b>${F(d)} más</b> (${p}%)`, color: 'var(--warning-soft)' }
        : { texto: `salieron <b>${F(d)} más</b> (${p}%)`, color: 'var(--sky)' };
};
...
export const cuadroBalance = (B) => {
    ...
    const filas = B.filas.map(f => {
        ...
        <tr style="border-bottom:1px solid rgba(var(--ink-rgb), 0.03);">
          <td style="padding:0.5rem 0.9rem; color:var(--text-strong); font-weight:700;">${esc(corta(f.marca))}</td>
          <td style="padding:0.5rem 0.9rem; width:26%;">
            ${barra(f.almacenado, 'var(--warning)')}<div style="height:3px;"></div>${barra(f.picado, 'var(--brand-light)')}
          </td>
    ...
    return `
      <div class="glass-panel" style="padding:0; overflow:hidden; border:1px solid rgba(var(--warning-rgb), 0.25);">
        ...
        <div style="padding:0.8rem 1.3rem; background:rgba(var(--shadow-rgb), 0.25); font-size:0.68rem; color:rgba(var(--ink-rgb), 0.4); line-height:1.8;">
          ...
          <br><span style="color:rgba(var(--ink-rgb), 0.3);">En ${B.dias_repartidos.length}
...
export const cuadroCobertura = (C) => {
    ...
    const lectura = (f) => {
        ...
        if (f.dias < 7) return { t: 'se queda sin piso', c: 'var(--danger-soft)' };
        if (f.dias > 90) return { t: 'ocupa piso sin rotar', c: 'var(--warning-soft)' };
