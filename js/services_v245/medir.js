...
export const resumen = (titulo) => {
    if (!ENCENDIDO || !tramos.length) return;
    const total = tramos.reduce((s, t) => s + t.ms, 0);
    const orden = tramos.slice().sort((a, b) => b.ms - a.ms);
    const lineas = orden.map(t => '  ' + String(t.ms).padStart(6) + ' ms   ' + t.nombre);
    console.log('⏱ ' + (titulo || 'Medición') + ' — ' + total + ' ms en '
...
export const midiendo = () => ENCENDIDO;
