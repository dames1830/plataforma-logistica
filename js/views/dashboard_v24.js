import { parseFile, parseBufferFiles, getAreaData, clearAreaData, generateKPIs, calculateBufferPallets, fetchBufferConfig, saveBufferConfig, logSystemAction, pingServer, saveBufferReport, loadBufferReport, fetchBufferHistory, dataStore, setDateFilter, currentDateFilter, getUploadMeta, initPersistentData, updateTablaTallas, getCol, getAreaLength } from '../services_v245/csvHub_v6.js?v=26.5.182';
// PULSE_ENGINE_V18_2_0_CLEAN_BUILD
import * as adminService from '../services_v245/adminService.js?v=26.5.182';
import { login as authLogin, getSession } from '../services_v245/auth.js?v=26.5.182';
import * as syncEngine from '../services_v245/sync_engine_v24_9.js?v=26.5.182';
import * as cyclicService from '../services_v245/cyclicCountService.js?v=26.5.182';
                    ...
                    <div style="flex-grow:1; overflow-y:auto; padding-bottom: 4.5rem;" id="nr_content_wrapper">
                        ${renderActiveTabContent(activeTab, capitalizedToday, pendingCount, totalCount)}
                            <div style="text-align: center; margin-top: 2rem; margin-bottom: 1.5rem; font-size: 0.65rem; color: rgba(255,255,255,0.25); font-weight: 700; letter-spacing: 0.05em;">
                                SYSTEM BUILD: v26.5.182 | MOBILE PORTAL
                            </div>
                    </div>
                        ...
                        <table style="width:100%; border-collapse:collapse; font-size:0.78rem;">
                            ...
                            <tbody>
                                ${(() => {
                                    ...
                                    // SELF-HEALING: If maestro is empty, fetch it from the server and re-render
                                    if (activeMaestro.length === 0) {
                                        fetch('https://logistics-backend-wv0x.onrender.com/api/logistics/articulos')
                                            .then(res => res.json())
                                            .then(resData => {
                                                if (resData && resData.data) {
                                                    dataStore.articulos = resData.data;
                                                    if (window.renderAlmacenajeTareas) {
                                                        window.renderAlmacenajeTareas(container);
                                                    }
                                                }
                                            }).catch(err => console.error("Error pulling articulos:", err));
                                    }
                                    ...
                                    if (activeMaestro.length > 0) {
                                        ...
                                        if (isArray) {
                                            ...
                                            let skuIdx = headers.indexOf('CODARTICULO');
                                            ...
                                            for (let i = 1; i < activeMaestro.length; i++) {
                                                const row = activeMaestro[i];
                                                if (!row) continue;
                                                const sku7 = String(row[skuIdx] || '').trim().substring(0, 7);
                                                if (sku7 && !liveGenderRimsMap.has(sku7)) {
                                                    liveGenderRimsMap.set(sku7, String(row[grIdx] || '').trim().toUpperCase());
                                                }
                                            }
                                        } else {
                                            activeMaestro.forEach(row => {
                                                ...
                                                if (sku7 && !liveGenderRimsMap.has(sku7)) {
                                                    liveGenderRimsMap.set(sku7, String(row[grKey] || '').trim().toUpperCase());
                                                }
                                            });
                                        }
                                    }
