  ...
  const runProcessBufferKPI = async (container, validarActivo, validarReserva, validarLPN, originalReserva, hasActivo, hasReserva, hasLPN, preCalculatedResults = null, fechaProceso = null) => {
    ...
    const renderRows = () => {
        ...
        uniqueLPNs.forEach(lpn => {
            const lpnRows = activeResults.filter(r => r.lpn === lpn);
            const allCompleted = lpnRows.every(row => row.generalState === "COMPLETADO");
            const allPending = lpnRows.every(row => row.generalState === "PENDIENTE");
            if (allCompleted) {
                lpnStatusMap[lpn] = "COMPLETADO";
            } else if (allPending) {
                lpnStatusMap[lpn] = "PENDIENTE";
            } else {
                lpnStatusMap[lpn] = "INCOMPLETO";
            }
        });

        const filtered = activeResults.filter(r => {
            const lpnStatus = r.lpn ? lpnStatusMap[r.lpn] : r.generalState;
            const matchesStatus = (filterValue === 'TODOS' || lpnStatus === filterValue);
  ...
  let ubicacionState = { page: 1, query: '', ubisArray: [] };
    ...
    const renderLayoutActivo = async (container) => {
      ...
      const buildLayoutHTML = (layoutData, stats, totalUnits, uniquePadresSize, targetContainer) => {
          ...
          // Recreate stats sets size for UI
