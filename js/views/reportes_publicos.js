...
} from '../services_v245/csvHub_v6.js?v=29.0718';

import * as adminService from '../services_v245/adminService.js?v=29.0718';
import { marcaNormalizada, marcaCorta, rotuloRango, selectorRango, diaOperativoDeTarea as diaOperativoCompartido } from '../services_v245/reportesComunes.js?v=29.0718';
import { datosMarcas, filasMarcas, cabeceraMarcas, armarTurnoDe, TEMA_CLARO } from '../reportes/marcas.js?v=29.0718';
import { renderLayoutActivo } from './public_layout_activo.js?v=29.0718';
import * as jornadaService from '../services_v245/jornadaService.js?v=29.0718';
...
import { CATALOGO as CAT_PUB, buscarSub, permisosDe as permisosPub }
    from '../services_v245/catalogoReportesPublicos.js?v=29.0718';
...
import { traerAreaPublicada } from '../services_v245/csvHub_v6.js?v=29.0718';
import { traerSellos, chipSello } from '../services_v245/selloService.js?v=29.0718';
...
import * as distribucionReporte from '../reportes/distribucion.js?v=29.0718';
...
async function init() {
  ...
  /* 2. La lista de grupos, PEDIDA AL SERVIDOR.
   *
   *    Antes se leía solo de memoria o del localStorage de esa PC, y si no
   *    estaba se caía a una lista escrita en el código con cuatro grupos. A
   *    Daniel le abría —su navegador la tenía guardada— y a cualquier otra
...
function renderShell(app) {
  ...
  app.innerHTML = `
    ...
    <div style="border-top:1px solid var(--border); background:var(--surface); padding:0.75rem 1.5rem; text-align:center; color:var(--text-muted); font-size:0.68rem; font-weight:600; letter-spacing:0.5px;">
      ...
      <span style="color:var(--text-muted); font-weight:500;">v29.0718</span>
    </div>`;
...
/* EL REPORTE ES EL MISMO QUE EL DE LA PLATAFORMA. Se le pasa lo único que este
   lado tiene distinto: su propia pantalla de "todavía no hay datos". */
distribucionReporte.configurar({
    hoy: () => getLogicalDate(),
    sinPublicar: (container, titulo, detalle) => {
        container.innerHTML = `<div class="report-card" style="text-align:center; padding:2.5rem;">
            <div style="font-weight:800; margin-bottom:0.4rem;">${titulo}</div>
            <div style="color:var(--text-muted); font-size:var(--t-sm); max-width:52ch; margin:0 auto;">
              ${detalle}<br>Todav&iacute;a no hay datos publicados: el robot los deja al cerrar el turno.
            </div>
          </div>`;
    },
});
...
async function renderDespachoModule() {
    if (currentSubTab === 'distribucion') {
        return distribucionReporte.renderDistribucion(document.getElementById('contentArea'));
    }
    if (currentSubTab === 'despacho_potencial') {
        return distribucionReporte.renderDespachoPotencial(document.getElementById('contentArea'));
    }
