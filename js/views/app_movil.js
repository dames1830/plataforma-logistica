...
import * as adminService from '../services_v245/adminService.js?v=29.0831';
import * as DES from '../services_v245/despachoCatalogo.js?v=29.0831';
import * as ORD from '../services_v245/despachoOrden.js?v=29.0831';
import * as jornadaService from '../services_v245/jornadaService.js?v=29.0831';
const BASE_API = (window.API_BASE_URL || 'https://logistics-backend-wv0x.onrender.com') + '/api/logistics';

import { armarLista, nombreCorto, nombreCompleto, claveDeOrden, iniciales } from '../services_v245/asistencia_comunes.js?v=29.0831';
import * as tareasComunes from '../services_v245/tareas_comunes.js?v=29.0831';
import * as metasService from '../services_v245/metasService.js?v=29.0831';
...
import * as temaService from '../services_v245/temaService.js?v=29.0831';
...
import { datosMarcas, armarTurnoDe } from '../reportes/marcas.js?v=29.0831';
import { marcaCorta } from '../services_v245/reportesComunes.js?v=29.0831';
...
import { arrancarDatosDelChat, alCambiarElChat, estadoDelChat, mandar, mandarConAdjunto,
         ...
         marcaDelMensaje, conversacionEnPantalla } from '../chat.js?v=29.0831';
...
import { LLAVE_AVISOS, puedeAvisos, mirarAvisos as mirarSuscripcion,
    ...
    from '../services_v245/avisos.js?v=29.0831';
...
/* EL BOTON DE CARGAR VA AL LADO DEL BUSCADOR y no escondido en el menu: es lo primero
   ...
   EL SELECTOR ACEPTA CUALQUIER ARCHIVO A PROPOSITO. Filtrar por extension parece mas
   prolijo y en Android es una trampa: cada explorador decide por su cuenta que tipo
   tiene un archivo, y los que bajan de WhatsApp muchas veces vienen como octet-stream.
   Con el filtro puesto, el archivo SIMPLEMENTE NO APARECE en la lista y el usuario no
   tiene forma de saber por que. Es mejor dejarlo elegir lo que sea y decirle "esto no
   parece un Excel" -que es lo que hace el lector- que esconderselo.

   Y OJO CON ESCRIBIR EL VALOR DEL ACCEPT ACA ADENTRO: este comentario vive dentro del
   BLOQUE DE CSS, y en CSS una barra con asterisco cierra el comentario donde aparezca.
   Al escribirlo, el comentario se corto a la mitad, el resto quedo como CSS invalido y
   la app se abrio en NEGRO. El chequeo de sintaxis no lo vio: como JavaScript, el
...
const prepararChat = async () => {
    ...
    /* QUE CONVERSACION ESTA EN PANTALLA. Daniel, 17-sep-2026: *"si lo veo en el móvil ya
       debería quitar ese aviso en la web"*. Lo que entra a la conversacion abierta se da por
       leido -si hay alguien mirando- y eso apaga el contador de la web. Antes la app no lo
       decia: un mensaje que llegaba con la conversacion abierta quedaba como no leido en los
       dos lados. Con la lista de integrantes del grupo encima, los mensajes no se ven. */
    conversacionEnPantalla(() => (raiz && seccion === 'chat' && chatSala && !gentePantalla) ? chatSala : null);
