 ...
 */
import { getSession, logout } from './services/auth.js?v=24.4.9';
import * as adminService from './services/adminService.js?v=24.4.9';

class App {
  constructor(rootId) {
    this.root = document.getElementById(rootId);
    this.APP_VERSION = 'v24.4.9';
  ...
  async render(user) {
    ...
    try {
        if (user) {
            const { renderDashboard } = await import(`./views/dashboard_v24.js?v=24.4.9`);
            this.root.innerHTML = '';
